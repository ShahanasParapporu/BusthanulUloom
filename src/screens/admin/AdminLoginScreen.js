...
// After login, just dispatch loginSuccess — RootNavigator handles showing Main.

import React, { useState } from 'react';
import {
  View, StyleSheet, KeyboardAvoidingView, Platform,
  Text, TouchableOpacity, StatusBar, ScrollView,
} from 'react-native';
...
const AdminLoginScreen = ({ navigation }) => {
  ...
  const showError = (msg) => { setSnackbarMessage(msg); setSnackbarVisible(true); };
  ...
  const handleLogin = async () => {
    ...
    if (
      ...
      // Dispatching sets isAuthenticated=true → RootNavigator shows Main
      dispatch(loginSuccess({ user: adminUser, role: USER_ROLES.ADMIN, token: 'admin-token' }));
    } else {
...
const styles = StyleSheet.create({
  ...
  topAccent2: {
    ...
    backgroundColor: THEME.primaryLight, opacity: 0.3,
  },
  ...
  warningBox: {
    ...
    marginBottom: 16, borderLeftWidth: 3, borderLeftColor: '#FF8F00',
  },
  warningText: { color: '#E65100', fontSize: 13 },
  loginButton: {
    marginTop: 8, borderRadius: 16, backgroundColor: THEME.primary, elevation: 4,
    shadowColor: THEME.primary, shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.3, shadowRadius: 5,
  },
  buttonContent: { height: 56 },
  loginButtonLabel: { fontSize: 18, fontWeight: 'bold', textTransform: 'none' },
  securityNote: { textAlign: 'center', color: '#9E9E9E', fontSize: 12, marginTop: 24, lineHeight: 18 },
