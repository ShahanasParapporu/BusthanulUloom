// src/screens/parent/ParentPortalScreen.js
// Uses AppHeader (same as HomeScreen) so the top bar is identical across tabs.
import React from 'react';
import {
  View, ScrollView, StyleSheet, Text, TouchableOpacity, Dimensions,
} from 'react-native';
import { Card, ProgressBar, Avatar, IconButton, Surface } from 'react-native-paper';
...
const CHILDREN_DATA = [
  {
    id: '1', name: 'Mohammed Rizwan', class: 'Grade 10 - B',
    attendance: 0.92, performance: 0.85,
    image: 'https://i.pravatar.cc/150?u=rizwan',
  },
  {
    id: '2', name: 'Aisha Fathima', class: 'Grade 7 - A',
    attendance: 0.98, performance: 0.91,
...
const ParentPortalScreen = () => {
  ...
  return (
    <View style={styles.container}>
      {/* Same AppHeader as HomeScreen */}
      <AppHeader role={role} user={user} onLogout={performLogout} />

      <ScrollView showsVerticalScrollIndicator={false}>
        {/* Welcome strip */}
        <View style={styles.welcomeStrip}>
          <Text style={styles.welcomeText}>Parental Dashboard</Text>
          <Text style={styles.subText}>Welcome, {user?.name || 'Parent'}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  subText: { color: 'rgba(255,255,255,0.8)', fontSize: 14, marginTop: 4 },
  section: { padding: 20 },
